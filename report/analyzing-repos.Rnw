...
\begin{abstract}
  ...
  characterized by three conditions applied to the sequence of changes: a scale-free structure, long-distance
correlations, and so-called {\em pink} noise. In this paper, we use a new
  methodology to measure whether this self-organized critical (SOC)
  state actually exists. Instead of focusing in a single and
  long-running project as is usual, we will rather work on 16
different repositories with different sizes and ages, and use a novel, more natural and precise
methodology to examine the sizes of changes. Our research shows that in all
16 repositories analyzed, at least one of the SOC characteristics exists, and
...
% Antonio - Then, say what are the implications of this fact. ;)
% The implication is that it is more widespread than previously
% thought and that few initial conditions might be needed to reach
% it. - JJ
\end{abstract}
...
environments. The dynamics of self-organized criticality is sometimes compared to that of a sand pile \cite{paczuski1996avalanche}, in the sense that
...
happens in software development teams, mainly in open source. 
...
And, in fact, several researchers have
...
\emph{scale in the size of changes}
\cite{wu2007empirical,gorshenev2004punctuated,Merelo2016:repomining}, but in some cases they also exhibit \emph{long-range correlations} and a \emph{pink noise} \cite{szendro2001pink} in the power spectral density, with
\emph{noise} or variations with respect to the normal frequency
...
% Antonio - This is the definition of pink noise, rigth?

% Antonio - I have removed some italics and added others in order to remark the characteristics mentioned below. 

These are the three characteristics of systems in a critical self-organized state which will be examined in this paper, where we take a different approach to analyzing the repositories. 
% Antonio - A different approach to which?
Instead of fixed-time changes in size of the whole repository, we introduce a {\em natural} timeline which uses as a {\em
  tic} the commit itself, and consider either increases or decreases in size, not only increases. 
% Antonio - as it is usual? as it was previously done?
In fact, a refactorization where a number of lines is changes might not show up using the previous methodology, 
% Antonio - which one do you refer to? If it was presented in a previous paper, please add a reference.
while it will have an impact in this one. 
...
control system such as \emph{git}. In fact, some explanations have been offered via conservation laws \cite{6784340} and other usual complex network
...
tools needed to mine 
% Antonio - mine => Prof. Merelo's
repositories on GitHub \cite{merelo16:self}, in
...
person \cite{merelo16:slash} and papers written collaboratively \cite{merelo16:self}.
...
obtained results will be reported in Section \ref{res}
...
clarity from top left to bottom right, with actual coefficients and p-values for the fit 
