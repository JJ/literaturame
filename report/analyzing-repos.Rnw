...
\title{Studying self-organized criticality in code repositories}

\author{J. J. Merelo$^{1}$, P. A. Castillo$^{1}$, Antonio Mora$^1$, Mario
  Garc\'ia-Valdez$^{2}$, Carlos Cotta${^3}$ \\
\mbox{}\\
$^1$ Geneura Team, ETSIIT and CITIC, University of Granada (Spain),
{\tt \{jmerelo,pacv,amorag\}@ugr.es}\\
$^2$ Dept. of Graduate Studies, Instituto Tecnologico de Tijuana
(Mexico) {\tt mario@tectijuana.edu.mx}\\
$^3$ Dept. Lenguajes y Ciencias de la Computaci\'on, Universidad de M\'alaga (Spain), {\tt ccotap@lcc.uma.es}
}
...
% team is a difficult task without straightforward solutions that is why it is interesting to find
% out the mechanism underlying collaboration in open source projects and
% their dynamics, which  might not be explicit or organized in
% a top-down fashion, but emerge in a self-organizing way, reaching
% eventually a critical state. 
In this paper we examine, using a novel
and more nathral and precise methodology, several
repositories measuring the three conditions for this self-organized
critical (SOC) state when analyzing the size of changes in the state of the repository and
its temporal sequence: a scale-free structure, long-distance
correlations and so-called {\em pink} noise.  We will try to find the
role of several parameters to reach this critical state. In these
repositories examined at least one of the SOC features exists, and
some of them exhibit all three, which means that effectively they are
in this state. 
\end{abstract}
...
that describe complex systems in many different venues and
environments. The dynamics of self-organized criticality is sometimes compared to
that of a sand pile \cite{paczuski1996avalanche}, in the sense that
the actual shape tends 
to reach also a critical state, 
% Antonio - lo he puesto en cursiva la primera vez (arriba)
...
\cite{Merelo2016:repomining}. These are the three characteristics of
systems in a critical self-organized state which will be examined in
this paper, where we take a different approach to analyzing the
repositories. Instead of fixed-time changes in size of the whole
repository, we introduce a {\em natural} timeline which uses as a {\em
  tic} the commit itself,
and consider either increases or decreases in size, not only
increases. In fact, a refactorization where a number of lines is
changes might not show up using the previous methodology, while it
will have an impact in this one. 

% The current state of the project has obviously an influence in its
...
% open model of development; however, 
...
them in active development,  trying to find out %whether 
the telltale signs of self-organized criticality, as enumerated above. 
Previously we have applied these techniques to examining the software
repository for the Moose Perl library 
\cite{Merelo2016:repomining}, books written mainly by a single
person \cite{merelo16:slash} and papers written collaboratively \cite{merelo16:self}. In this paper we are going  % maybe a reference or footnote to this repo (https://github.com/moose) should be added?  - Pedro
%Added citation to the paper
to work on a total of 16 repositories, chosen to represent different
...
including art, documentation and building tools, we have focused on
the code written in the two or three main languages of the repository
itself, since in fact the dynamics of documentation of build tools is
probably completely different. 

The rest of the paper is organized thus: After presenting a brief state of the art next, 
followed by the methodology in Section \ref{sec:method},
obtained results  will be presented  in Section \ref{res}
and eventually we will expose our conclusions. 
...
ease. Indeed, this state of self-organized criticality
...
% Antonio - ?qu? es lo que se auto-organiza? 'themselves'? 'the code'?
% developers - JJ
which is
...
% Antonio - cita?
% Antonio, de veras que me ayudar?a si t? buscaras esas citas. No tienes m?s que irte a Google Scholar y poner "Self-organized criticality - Wikipedia" e insertarla aqu?. No tengo tiempo de mirar absolutamente nada ahora - JJ

Self-organization, eventually, might produce a critical state
given the necessary conditions. However, there has been no work going further and
proving this even in the case that work is done by a few people and
...
defined in \cite{benbya2006toward}. This paper presents 
% Antonio - se puede malinterpretar entre este art?culo y el de la cita. Yo lo pondr?a en pasado ('presented') para aclarar que es el citado
the seven
mechanisms that create complexity in systems as initially enunciated for
biological systems and apply them to information system
development. 

The principle of {\em change rate}, which appears in the
...
repositories is embedded in a kind of social network. \cite{valverde2007self}
...
of finding out how that complex network dynamics effectively emerge. 
% Antonio - pero qu? objetivo tiene eso? Se podr? mejora la forma de colaborar o algo as?? Creo que habr?a que comentarlo.
% Ya se ha hecho en la intro - JJ
Next we
...
We have chosen 16 
% Antonio - own? public?
repositories in different states of development, and representing from web frameworks such as Django to Atom plugins in the case of {\em language-perl6e}. They include many different languages, either interpreted or compiled, and vary also in {\em professionally}, from the aforementioned plugin to Docker (an open platform for building and running distributed applications), 
created and maintained by a professional community. 
% Antonio - Y las personas en s?? C?mo se mide su 'profesionalidad'? Explicarlo mejor
% Hay proyectos de gente y otros liberados por empresas. - JJ

% ¿ponemos el periodo de tiempo a lo largo del cual se han recogido los datos? 
% suponemos el mismo para todos los repositorios (comentario de un Revisor del GECCO) - Pedro

In this paper we will work with the size of changes to a particular
set of files in the repository, selected via wildcards. 
% Antonio - aclarar qu? quiere decir esto de las 'wildcards'
% Comodines - JJ
We exclude other 
...
% Antonio - 'diff' es un programa no?
% Tambi?n es un programa, pero no estoy hablando de eso aqu? - JJ
as
...
% Antonio - referencias de ejemplo
which takes
...
The $x$ axis for these timelines does not correspond to physical time,
...
our research methodology, which considers discrete 
% Antonio - discrete -> simple? / atomic? Los cambios diarios no ser?an tambi?n discretos (no continuos)?
changes, to papers such
...
be better detected using commits. An {\em avalanche} caused by a small change happening in a day followed by a flurry of changes fill appear as a big change if the time unit is bigger than the discrete commit. 
...
\caption{Summary of statistical measures for the software repositories we have
  analyzed here. {\tt age} is measured in total number of
  commits that affects the files checked. The complete name is shown here,
  which includes the depth of the directory structure as a number and
  the extension. For instance, the first one has directories up to deep 4
  and uses CoffeeScript ({\tt coffee}) and Perl 6 {\em p6}. They are all open source and can be found in the GitHub repository with the same name.\label{t:stat}}
\end{table*}
%
% Antonio - Antes de comparar resultados, una duda que puede surgir es c?mo se justifica la selecci?n de ficheros para que sean comparables entre distintos repos??? Explicarlo aqu? o en lo de las wildcards.
%
This table shows that, at least from a macro point of view, median and
means are remarkably similar among themselves and with other repositories that have been examined. Median changes are never bigger than 100 lines and it can be as small as 6 or 9 lines. Means span a bigger range, from around 22 lines to several tens of thousands in the case of Tensorflow. The fact that the mean is so
...
@ 
\caption{Timeline of changes for all repositories with lines smoothed over 20 and 10 changes, shown in different colors.\label{fig:smoothie}}
\end{figure*}
...
\end{figure*}
%
This chart shows what seems to be a Pareto distribution,
% Antonio - ref? 
% That's something you can do yourself, right? - JJ
 with the commit sizes
...
different practices by different authors, but also to different in-company practices. It is specially remarkable the {\em fat tail} in the case of Tensorflow and {\em ejabberd}, next-to-last row, first and second from right. Due to some practices of merging a whole branch in a single commit, they exhibit exceptionally big commit sizes, with tens of thousands lines. In that case, that would show that there is some top-down organization taking place, but it does not hide the general self-organizing shape of the curve to their left. 
...
number of changes or the size of the project. The shape of the line in
...
\caption{Changes, ordered by size, and represented in a logarithmic
  $y$ axis. Side by side, the histogram and Zipf chart for the four papers analyzed.\label{fig:zipf}} % for the 16 repositories   ?
\end{figure*}
%
%
We can in fact fit this to a Zipf distribution, whose Zipf exponents and intercepts for these models are shown in Table \ref{t:zipf},
and are of the same order, but different range, of those found  in
...
number of changes of the paper, as it has been shown above. 
...
found in \cite{Merelo2016:repomining}, are present here too. In the mentioned paper, there was positive
...
Once two of the three features of self-organized criticality 
% Antonio - las tres caracter?sticas no se han comentado previamente...?ponerlas en la intro?
% _Est?n_ en la intro. Le he a?adido una frase que menciona que son las tres caracter?sticas que se estudian - JJ
have been
proved, at least in some of the projects, we will
focus on the third, the presence of \emph{pink noise}, as measured by
the power spectral density, which we show in Figure \ref{fig:spectrum}. The presence of {\em pink noise} would be characterized by a spectrum with a negative slope, with decreasing power the higher the frequency. 
%
\begin{figure*}[h!tb]
  \centering
...
clarity from top left to bottom right, with actual coefficients and p-values for the fit are
...
In this paper we were interested in finding traces of self-organized criticality in software repositories by looking for certain features that are peculiar to the critical state: scale-free behavior, long-distance correlations and pink noise. Our methodology, which is new in the sense that we have used counts size of commits as a discrete measure, will show a much clearer picture of the state
...
% Antonio - I agree. The people involved in repos are very important, as they are the reason of self-organization indeed. ;)
% Interesting discussio, not for this paper - JJ

After analyzing the software repositories we can conclude that, in
fact, all repositories analyzed show some of the features, specially
...
amount of interaction needed before the critical state is actually
reached. From the limited amount of repositories we have studied, we
could put this number between 100 and 1000 changes, with all
repositories examined over 1000 changes exhibiting self-organized
criticality. In particular, this would indicate that the only
condition needed for the critical state to arise is the age of the
repository, or maybe its size. On the other hand, there are some
differences depending on the software development methodology which
might impact the evolvability of the repository, as has been seen in
the cases of repositories such as Tensorflow. Even so, the fact that
they are in the SOC state and evolving continuously shows that a SOC
state might be a good indication of a high fitness in the complex
social system that develops the repository, which is what we were
looking for. 
...
repositories, specially in the boundary where we think
self-organization arises, between 100 and 1000 commits, and these with other
...
than the substrate itself, to different types of collaboration. We
...
size. A study of particular circumstances of every repository will
also help us to understand what self-organization means and, finally,
as was our initial objective, if this fact can be used to create
methodologies that improve productivity in work teams and to develop a
fitness measure for repositories based on self-organized criticality
that allows evolution via genetic improvement.  


\section*{Acknowledgements}

This work has been supported in part by: de Ministerio espa\~{n}ol de Econom\'{\i}a y Competitividad under project TIN2014-56494-C4-\{1,3\}-P (\{UMA,UGR\}-EPHEMECH). 
% Antonio - completar con otros, para la versi?n final (KNOWAVES, por ejemplo)
