...
\begin{abstract}
  In order to improve team productivity and the team interaction itself, as well as the willingness of occasional volunteers, it is interesting to find 
  ...
  we can deduce some rules on how to improve it.
...
eventually try 
% Antonio - trying?
to find if {\em age} as measured as the number of changes 
...
In open source projects there is usually a {\em core} team with has the
...
users. Besides, many software projects have a series of participation guidelines indicating where and how users can spontaneously contribute
to the code. These contributions are incorporated, for instance, through the so-called {\em pull requests} in {\em git}, which means that the contributor {\em requests} the core team to {\em pull} or incorporate changes from her own repository.
...
which eventually get merged with master. 
...
Best practices usually dictate that changes, represented by {\em commits}, should be atomic, i. e., affect a single function or file and be independently testable, with the object that if something goes wrong the guilty commit can be
...
The dynamics of self-organized criticality is sometimes compared to that of a sand pile \cite{paczuski1996avalanche}, in the sense that the actual shape tends
...
to work on a repository for several papers in which our research group has been working for different amounts of time, from a few months to more than a
