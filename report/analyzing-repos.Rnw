...
% Antonio - ?esto se refiere a un algoritmo evolutivo (PG) para mejorarlo o es una expresi?n hecha? Aclarar si es el primer caso.
...
\cite{wu2007empirical,gorshenev2004punctuated,Merelo2016:repomining,gao2014analysis} and  
...
% Antonio - a qu? modelo te refieres?
...
changes getting a smaller spectral density \cite{Merelo2016:repomining}. These are the three characteristics of systems in a critical self-organized state which will be examined in this paper.
...
% Antonio - no queda clara la motivaci?n del art?culo. ?por qu? es necesario este estudio? ?qu? objetivo/beneficio persigue?
...
After presenting a brief state of the art next, 
