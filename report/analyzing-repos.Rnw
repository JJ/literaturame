...
this is a challenge a bigger than simply counting bugs eliminated or
tests passed. 

In this paper, we take the first steps towards the evaluation and
eventual improvement of software projects by treatubg them as complex
...
for (i in 1:length(lines) ) {
    ...
    spec.fit.df <- rbind( spec.fit.df,
                         data.frame( Name = repo[[1]][1],
                                    Intercept = summary(spec.fit)$coefficients[1],
...
\vspace*{2mm}
\caption{Summary of coefficients of the linear models adjusting the
  power spectrum. Pink noise would correspon to a coefficient of -1.\label{t:zipf}}
\end{table}
