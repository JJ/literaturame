...
\caption{Summary of statistical measures for the software repositories
 ...
 files under study, next is the main language the project uses, {\tt
   Mean} and {\tt Median} are computed over change sizes, with {\tt SD}
 referring to the standar deviation from the mean.\label{t:stat}}
\end{table*}
%
...
\caption{Timeline of changes for all repositories with lines smoothed
  ...
  repositories is the same as in Table \ref{t:stat}, and will be kept
  across the rest of the figures. Please note the log scale for the
  $y$ axis, which represents change size. $x$ axis runs through time. \label{fig:smoothie}}
\end{figure*}
% one of the GECCO reviewers said: "The text in figures 1, 2, 3, 4, and 5 is not readable".    -   pedro
% It is not needed, actually. The best is probably to supress it - JJ
