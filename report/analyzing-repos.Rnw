...
\begin{abstract}
  In order to improve team productivity interactions  within the team
  ...
  we can deduce some rules on how to interactions and thus productivity.
...
on a wider range of repository sizes and ages, not only big and {\em
  old ones}, with two main objectives: establish their self-organized
criticality state or at least some features of it, and
eventually try 
% Antonio - trying?
...
The measures in these repositories we can conclude that the number
