...
\begin{abstract}
  ...
  repositories are in a particular state, reached via self-organization, and what are the conditions
  to reach that state will yield some insights on this process, and, from this,
  we can deduce some rules on how to improve it. In this paper we will focus on the former,
  examining repositories, hosted in GitHub, for open source projects,
  and taking a series of measures to see if they show the characteristics of a critical  
  state, which can be deduced by the 
existence of a scale-free structure, long-distance correlations and
{\em pink}
noise when analyzing the size of changes in the state of the
repositorio and its temporal sequence.  Our
...
projects, such as software or even literary works; in this paper we
apply a new methodology to measure changes and their sequence, focus
on all types of repositories, not only big and {\em old ones}, and
eventually try to find if {\em age} as measured by the number of
changes is the only condition needed to find self-organization.
\end{abstract}
...
software repositories has been well established \cite{wu2007empirical,gorshenev2004punctuated,Merelo2016:repominingANON,gao2014analysis} and 
...
further changes down the line in a regular pattern. The anecdotal
evidence cathered by the authors confirms this kind of events
happening in software development teams, mainly in open source. In
open source projects there is usually a {\em core} team with has the
...
users. Besides, many software project have a series of contribution
guidelines indicating where and how users can spontaneously contribute
to the code. These contributions are incorporated through {\em pull
  requests}, which means that the contributor {\em requests} the core
team to {\em pull} or incorporate changes from her own repository.
...
management tools based in a synchronize from
...
which eventually get merged with master. Best practices usually
dictate that changes, represented by {\em commits}, should be atomic,
i. e., affect a single function or file and be independently testable,
with the object that if something goes wrong the guilty commit can be
...
in the whole repository used by other studies. 

The dynamics of self-organized
...
opposed from the the lack of this quality in software created by a
...
\cite{Merelo2016:repominingANON} and books written mainly by a single
person \cite{merelo16:slashANON}, in this report we are going  % maybe a reference or footnote to this repo (https://github.com/moose) should be added?  - Pedro
...
of a previous report \cite{merelo16:selfANON} has been chosen since it has
...
that also hosts this paper (hidden now for double blind review).  % maybe we should add here the URL to the paper repo, as a footnote  - Pedro


% in this figure, the different subplots should be identified (maybe including the name of repo in each case)  - Pedro
% I am going to eliminate it. It should have done so automatically. 
% \begin{figure*}[h!tb]
%   \centering
...
\begin{figure*}[h!tb]
  \centering
<<smoothie,message=FALSE, cache=FALSE,echo=FALSE,warning=FALSE,fig.height=7,fig.subcap=c('DCAI','PPSN','Book Prediction','Volunteer computing'),out.width='.115\\linewidth'>>=
for (i in 1:length(lines) ) {
...
\begin{figure*}[h!tb]
  \centering
<<linecount,message=FALSE, cache=FALSE,echo=FALSE,warning=FALSE,fig.height=4,fig.subcap=c('DCAI','PPSN','Book Prediction','Volunteer computing'),out.width='.115\\linewidth'>>=
...
\begin{figure*}[h!tb]
  \centering
<<powerlaw,message=FALSE, cache=FALSE,echo=FALSE,warning=FALSE,fig.height=4,fig.subcap=c('DCAI','PPSN','Book Prediction','Volunteer computing'),out.width='.245\\linewidth'>>=
...
\begin{figure*}[h!tb]
  \centering
<<autocorrelation,message=FALSE, cache=FALSE,echo=FALSE,fig.height=4,fig.subcap=c('DCAI','PPSN','Book Prediction','Volunteer computing'),out.width='.115\\linewidth'>>=
for (i in 1:length(lines) ) {
...
\begin{figure*}[h!tb]
  \centering
<<spectrum,message=FALSE, cache=FALSE,echo=FALSE,fig.height=4,fig.subcap=c('DCAI','PPSN','Book Prediction','Volunteer computing'),out.width='.115\\linewidth'>>=  
for (i in 1:length(lines) ) {
