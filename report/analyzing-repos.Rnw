...
files <- age[order(age$age),]
lines <- list()

for (i in 1:length(files$Name) ) {
# for testing here
    file.name <- paste0(pref,files[[i,'Name']],'.csv')
    lines[[i]] <- read.csv(file.name) 
    files[[i,'Mean']] <- as.double(mean(lines[[i]]$Lines.changed))
    files[[i,'Median']] <-  as.double(median(lines[[i]]$Lines.changed))
    files[[i,'SD']] <-  as.double(sd(lines[[i]]$Lines.changed) )
...
hierarchical rules and self-organization emerging from the bottom, which make software development teams a complex social system.

In this paper we will mine software repositories and take the measures
that would confirme they are in a self-organized critical
...
self-organized criticality state of the repositories under study.
Next we
...
@
\vspace*{1mm}
\caption{Summary of statistical measures for the software repositories we have analyzed here. {\tt age} is measured in total number of
  commits that affect the files under study. The complete name is shown here,
...
the cases of repositories such as Tensorflow. 
...
as was our initial objective, if this fact can be used to design
methodologies that improve productivity in work teams.  
