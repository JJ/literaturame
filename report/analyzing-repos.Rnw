...
critical state in software repositories is supported by several macro measures that certify the non-existence of a particular
...
% The definition of pink noise is in the reference next to the words
% "pink noise" - JJ

Even if the existence of this SOC state in repositores is well
...
In fact, a refactorization where a number of lines is changes might
not show up in daily changes, 
...
person \cite{merelo16:slash} and papers written collaboratively
\cite{merelo16:self}; they do exhibit some of the signs that indicate
we are dealing with a complex system, such as a scale-free size of
changes. 
