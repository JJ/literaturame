...
\begin{abstract}
  Software development teams become eventually complex systems,
  reaching a critical state. In this paper we use a different
  methodology to measure whether this self-organized critical (SOC) state actually exists by looking
  at the three conditions: a scale-free structure, long-distance
correlations and so-called {\em pink} noise. We will work on 16
different repositories, and use a novel, more natural and precise
methodology to examine change sizes. Our research shows that, in all
16 repositories examined, at least one of the SOC features exists, and
most of them exhibit all three, which leads us to think that SOC is
more widespread in software development than previously thought and
...
happen in software development teams, mainly in open source. And, in fact, several researchers have
