...
\title{What is the fitness of a software project? Studying self-organized criticality in code repositories}
...
%   \author{J. J. Merelo\inst{1}, P. A. Castillo\inst{1}, Mario GarcÃ­a-Valdez\inst{2}}
...
team is a difficult task without straightforward solutions.  In order
to improve team productivity and interactions within the team as well
as the willingness of occasional volunteers, it is interesting to find
out the mechanism underlying collaboration in open source projects and
their dynamics. These mechanisms might not be explicit or organized in
a top-down fashion, but arise from the collaboration and the way it is
done, being thus self-organizing. This is why finding if open source
code repositories are in a particular, critical, state, reached via
self-organization, and what are the conditions to reach that state
will yield some insights on this process, and, from this, we can
deduce some rules on how to interactions and thus productivity.  In
this paper we will focus on trying to find this self-organized
criticality which makes the project a complex system and implies
certain rules on its future evolution, state by examining
repositories, hosted in GitHub, for open source projects, and taking a
series of measures to see if they show the characteristics of a
critical state, which can be deduced by the existence of a scale-free
structure, long-distance correlations and so-called {\em pink} noise
when analyzing the size of changes in the state of the repository and
its temporal sequence.  Our intention is to prove that, although with
entirely different characteristics, teams, rules and sizes, most
repositories independently of the number of collaborators and their
real nature, self-organize, which implies that it is the nature of
these interactions, and not the object of the interaction, which takes
the project to a critical state. This critical state has already been
...
also focus on a wider range of repository sizes and ages, not only big
and {\em old ones}, with two main objectives: establish their
self-organized criticality state or at least some features of it, and
eventually try and find if {\em age} as measured as the number of
changes is the only condition needed to find self-organization, since
all repositories examined show at least some features and all of them
the three main features.  Taking into account these measures in the
repositories we can conclude that the number of changes is the main,
if not only, factor that creates a critical state in software
repositories.
% Añadido - JJ
\end{abstract}
...
% Antonio - ¿esto se refiere a un algoritmo evolutivo (PG) para mejorarlo o es una expresión hecha? Aclarar si es el primer caso.
must eventually deal with this complexity. But
...
mainly large, long-running projects, of a self-organized {\em critical
state} (SOC State) \cite{bak1988self} in  
...
but the fact is that the model 
% Antonio - a qué modelo te refieres?
...
changes getting a smaller spectral density \cite{Merelo2016:repominingANON}. These are the three characteristics of systems in a critical self-organized state which will be examined in this paper.
...
% Evolvability es bueno, porque lo hace más flexible a cambios - JJ
...
% Antonio - no queda clara la motivación del artículo. ¿por qué es necesario este estudio? ¿qué objetivo/beneficio persigue?
...
obtained \protect\hyperlink{res}{results} will be presented 
...
% Antonio - ¿qué es lo que se auto-organiza? 'themselves'? 'the code'?
% developers - JJ
which is
...
% Antonio - cita?
% Antonio, de veras que me ayudaría si tú buscaras esas citas. No tienes más que irte a Google Scholar y poner "Self-organized criticality - Wikipedia" e insertarla aquí. No tengo tiempo de mirar absolutamente nada ahora - JJ
...
% Antonio - se puede malinterpretar entre este artículo y el de la cita. Yo lo pondría en pasado ('presented') para aclarar que es el citado
the seven
...
% Antonio - pero qué objetivo tiene eso? Se podrá mejora la forma de colaborar o algo así? Creo que habría que comentarlo.
...
% Antonio - Y las personas en sí? Cómo se mide su 'profesionalidad'? Explicarlo mejor
...
% Antonio - aclarar qué quiere decir esto de las 'wildcards'
% Comodines - JJ
We exclude other 
...
% También es un programa, pero no estoy hablando de eso aquí - JJ
as
...
% Antonio - discrete -> simple? / atomic? Los cambios diarios no serían también discretos (no continuos)?
...
\end{table*}
%
% Antonio - Antes de comparar resultados, una duda que puede surgir es cómo se justifica la selección de ficheros para que sean comparables entre distintos repos??? Explicarlo aquí o en lo de las wildcards.
%
...
found in \cite{Merelo2016:repominingANON}, are present here too. In the mentioned paper, there was positive
...
% Antonio - las tres características no se han comentado previamente...¿ponerlas en la intro?
% _Están_ en la intro. Le he añadido una frase que menciona que son las tres características que se estudian - JJ
have been
...
% Antonio - completar con otros, para la versión final (KNOWAVES, por ejemplo)
Several projects \\
