...
application, task assignment systems or mailing lists. These are
probably just some of the self-organization processes taking place,
but the fact is that SOC
predicts that the system will reach a
...
person \cite{merelo16:slash} and papers written collaboratively \cite{merelo16:self}. In this paper we are going  
to work on a total of 16 repositories, chosen among those that have at
least a few hundred commits, independently of the actual time it has taken to
...
defined in \cite{benbya2006toward}, which presents 
the seven
...
web frameworks such as Django to Atom plugins in the case of {\em
  language-perl6e}. They include many different languages, either
interpreted or compiled, and vary also in {\em professionality}, from
the aforementioned and more amateurish plugin to Docker (an open platform for building and running distributed applications), 
...
that have been made to the considered files. All data were collected
in the same dates at the beginning of 2017; however, since we are not
comparing repositories among them, in principle the fact that data has
been collected at the same time or not should not have any influence
on the results. 

Since changes include both the insertion and deletion of lines in
...
@ 
\caption{Timeline of changes for all repositories with lines smoothed
  over 20 and 10 changes, shown in different colors. The order of the
  repositories is shown increasing in age from the top left to bottom
  right. This order will be kept for the rest of the figures. \label{fig:smoothie}}
\end{figure*}
...
\caption{Changes, ordered by size, and represented in a logarithmic
  $y$ axis. Side by side, the histogram and Zipf chart for the 16 repositories analyzed.\label{fig:zipf}}
\end{figure*}
%
%
...
@ 
\caption{Autocorrelation plot. The horizontal lines show the level of statistical significant, with any line surpassing it indicating positive or negative self-correlation.\label{fig:auto}}
\end{figure*}
% 
...
\vspace*{2mm}
\caption{Summary of coefficients of the linear models adjusting the
  power spectrum with the p-value of the fit. It is considered pink
 noise if the values are between 0 and -2. In the cases that
