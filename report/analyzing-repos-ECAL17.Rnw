...
%
%
We can in fact fit these charts to a Zipf distribution using a linear
model that fits the logarithm of the change size to the position. The
Zipf exponents and intercepts for these models are shown in Table
\ref{t:zipf}, and are of the same order, but different range, of those
