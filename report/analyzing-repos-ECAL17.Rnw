...
\citep{Merelo2016:repomining}, books written mainly by a single
person \citep{merelo16:slash} and papers written collaboratively
\citep{merelo16:self}; they do exhibit some of the signs that indicate
...
network. \citet{valverde2007self}, for instance, 
...
for (i in 1:length(lines) ) {
    ...
    print(ggplot(lines.count, aes(x=Lines.changed, y=count))+geom_point()+scale_x_log10()+scale_y_log10()+stat_smooth() + theme(legend.position="none",axis.title.x=element_blank(),axis.title.y=element_blank()))
}
@ 
\caption{Number of changes vs lines changed in a log-log scale.\label{fig:changes}}
\end{figure*}
%
...
%
%
We can in fact fit these charts to a Zipf distribution, whose Zipf exponents and intercepts for these models are shown in Table \ref{t:zipf}, and are of the same order, but different range, of those found by
\citet{merelo16:slash}, where it hovers around 6 for the intercept and
-0.01 for the slope.
%
\begin{table}[h!tb]
    \centering
...
the power spectral density, which we show in Figure \ref{fig:spectrum}. The presence of {\em pink noise} would be characterized by a spectrum with a log-log slope equal to $-1$, with power decreasing inversely proportional to frequency. 
%
\begin{figure*}[h!tb]
  \centering
...
for (i in 1:length(lines) ) {
    ...
    print(autoplot( this.spectrum ) + scale_x_log10() + theme(legend.position="none",axis.title.x=element_blank(),axis.title.y=element_blank()) )
...
@ 
\caption{Spectral density of changes, power vs. frequency. The repos are in the same order as above.  \label{fig:spectrum}}
\end{figure*}
%
\begin{table}
    \centering
...
coefficient values around $-1$, and that is the case for all of
