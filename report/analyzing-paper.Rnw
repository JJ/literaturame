...
itself and through messages in other communication media, such as the Slack or IRC chat
...
experience of software developers and paper writers tnat experience
...
this paper we are interested in finding out whether these mechanism
are exclusive or software teams or if, indeed, self-organized
...
hosts also data and, like in the case of this particular paper, code
...
for different amounts of time, from a few months to moreo than a
...
\cite{Castillo2016books}. The other three papers chosen is in one case
...
commands in the case of the paper using Knitr. The inclusion of lines
...
% Reference 
% Added - JJ
In fact, this self-organization has been also observed in similar
projects such as Wikipedia 

This self-organization, eventually, might produce a critical state
given necessary conditions. However, there has been no work going further and
...
In this paper we will work with the sizes of changes to a particular
...
the bigger of these two values; the main reason for doing so is also
...
using the biggest of these two values as change size. 
...
that also hosts this paper. The sequence of changes for the 4 files is
shown in figure \ref{fig:tl}. 
%
\begin{figure*}[h!tb]
  \centering
...
The $x$ axes for these timelines does not correspon to physical time,
but simply to sequence. In this sense, there is a difference between
our line of research, which considers discrete changes, to papers such
...
is different, much more skewed towards bigger sizes, while the
software library analyzed in \cite{Merelo2016:repominin} had a median
