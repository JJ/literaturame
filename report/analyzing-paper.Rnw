...
information from the repository and how we have processed it. After
presenting a brief state of the art next, \protect\hyperlink{res}{results} will be presented in the next section,
and eventually we will expose our
\protect\hyperlink{conc}{conclusions}. 
...
of number of users, age and type of information they hold implies that
self-organization, as should be expected, is achieved with relative
easiness. In fact, this state of self-organized criticality
quantitatively proves what has already been established via
...
teams. Self-organized criticality arise from
...
on repositories that are not devoted to software.


In this paper we will examine different repositories of the same kind,
all devoted to the collaborative writing of scientific papers, but
with different age, in order to try and find out if self-organization
...
A summary of the statisticas characteristics of size of the commits,
...
averages are remarkably similar, with median between 9 and 22 lines
and average between 24 and 54. The fact that the average is so
separated from the median already is a hint that this is a skewed
distribution. 
...
\begin{figure*}[h!tb]
  \centering
<<timeline, cache=FALSE,echo=FALSE,fig.height=7,fig.subcap=c('PPSN','Book Prediction','DCAI','Volunteer computing'),out.width='.245\\linewidth'>>=
...
\begin{figure*}[h!tb]
  \centering
<<smoothie,message=FALSE, cache=FALSE,echo=FALSE,warning=FALSE,fig.height=7,fig.subcap=c('PPSN','Book Prediction','DCAI','Volunteer computing'),out.width='.245\\linewidth'>>=
for (i in 1:length(lines) ) {
...
\begin{figure*}[h!tb]
  \centering
<<linecount,message=FALSE, cache=FALSE,echo=FALSE,warning=FALSE,fig.height=8,fig.subcap=c('PPSN','Book Prediction','DCAI','Volunteer computing'),out.width='.245\\linewidth'>>=
...
\begin{figure*}[h!tb]
  \centering
<<powerlaw,message=FALSE, cache=FALSE,echo=FALSE,warning=FALSE,fig.height=8,fig.subcap=c('PPSN','Book Prediction','DCAI','Volunteer computing'),out.width='.245\\linewidth'>>=
...
\begin{figure*}[h!tb]
  \centering
<<autocorrelation,message=FALSE, cache=FALSE,echo=FALSE,fig.height=8,fig.subcap=c('PPSN','Book Prediction','DCAI','Volunteer computing'),out.width='.245\\linewidth'>>=
for (i in 1:length(lines) ) {
...
\begin{figure*}[h!tb]
  \centering
<<spectrum,message=FALSE, cache=FALSE,echo=FALSE,fig.height=8,fig.subcap=c('PPSN','Book Prediction','DCAI','Volunteer computing'),out.width='.245\\linewidth'>>=  
for (i in 1:length(lines) ) {
