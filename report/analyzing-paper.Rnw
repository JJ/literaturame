...
are bigger when fiction is being written, which also matches the macro
averages and medians observed above. 


The scale free nature of the work in the repository can be more
properly observed by looking at the changes in some other way, ranking changes by size
